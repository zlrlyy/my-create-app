...
import { removeUserCookies } from './util';
import { isServer } from '@/utils/typeHelpers';
import Qs from 'qs';
...
export const checkToken = (code: string | number) => {
  ...
  // 没有权限
  if (
    code === ERRORCODE.VALID_CODE.HTTP_403_AUTHENTICATION_EXPIRED ||
    code === ERRORCODE.VALID_CODE.HTTP_403_AUTHENTICATION_FAILED
  ) {
    bool = false;
    removeUserCookies();
    if (!isServer()) {
...
instance.defaults.paramsSerializer = function (params: any) {
...
export function unauthHandler() {
  if (isServer()) {
    // sever-side
    // message.xxx while throw `document is undefined`
    removeUserCookies();
    // eslint-disable-next-line
    clearAuth();
  } else {
    // Make sure there is only one message
    // when multi-request tokens expired
    message.destroy();
    message.error({
      key: 'token-expired',
      content: '登录超时，请重新登录',
...
async function request<T>(url: string, config: RequestConfig): Promise<T> {
  ...
  try {
    ...
    const { error_code: errorCode, data: result } = res.data;
    ...
    if (isAxiosError(err)) {
      ...
      const { error_code: code, error_message: message, data } = err;
